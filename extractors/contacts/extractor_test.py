...
def extract_many(texts: list[str]) -> list[Contacts]:
  return ce.extract_many([normalize(text) for text in texts])

def extract(text: str) -> Contacts:
  return ce.extract(normalize(text))

def describe_ContactExtractor() -> None:
