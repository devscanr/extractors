...
from ..utils import Skill, MaybeSkill, contextual, neighbour, contextual_or_neighbour
...
SKILLS: list[Skill] = [
  ...
  # .NET
  Skill(".NET", [ver1(".net"), "dotnet", "dot.net"]),

  Skill("ASP.NET", ["asp.net", "asp"]), # framework to build web apps and services with .NET and C#
  Skill("ML.NET", ["ml.net"]),          # ML framework for .NET
  Skill(".NET MAUI", [".net maui", "maui"]), # multi-platform app UI, evolution of Xamarin.Forms
  Skill("Blazor", ["blazor"]), # fullstack framework within .NET (Blazor WebAssembly for client-side web apps and Blazor Server for server-rendered web apps.)
  Skill("Unity", ["unity-engine", "unity=platform", "unity=3d", propn("unity")]), # gamedev engine
  MaybeSkill("Unity", ["unity"], disambiguate=ctxn), # /
  ...
  MaybeSkill("Microsoft-365", ["365"], disambiguate=ctx),
